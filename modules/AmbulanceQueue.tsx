...
import { ambulanceQueue, AMB_DOCS, AMB_PHOTOS, Ambulance } from "@/data/ambulances";
...
const missing = (a: Ambulance) => {
  const m: Record<string, string> = { oxygen: "O2", aed: "AED", stretcher: "Stretcher", monitor: "Monitor" };
  return Object.keys(m).filter((k) => !(a.equip as any)[k]).map((k) => m[k]);
};
...
export function AmbulanceQueue(_: ModuleProps) {
  ...
  const openAmb = (a: Ambulance) => { const o: Record<string, { status: DocStatus; reason?: string }> = {}; AMB_DOCS.forEach((d) => (o[d] = { status: "verified" })); setDocMap(o); setSel(a); };
      ...
      <div className="card" style={{ padding: 0 }}>
        ...
        <DataTable<Ambulance>
          ...
          columns={[
            ...
            { key: "equip", label: "Equipment", render: (a) => { const m = missing(a); return m.length === 0 ? <span style={{ color: "#475569", fontWeight: 500 }}>✓ Complete</span> : <span style={{ color: "#B45309", fontWeight: 500 }}>⚠ {m.length === 1 ? "Missing " + m[0] : "Missing Equipment"}</span>; } },
      ...
      <SideDrawer open={!!sel} onClose={() => setSel(null)} title="Ambulance Review" footer={sel && <>
        ...
        {sel && <>
          ...
          <div style={{ padding: "10px 22px 18px" }}>
            ...
            <Sec>Documents</Sec>
            <DocVerification
              docs={AMB_DOCS.map((d) => ({ name: d, uploadDate: sel.year, status: docMap[d]?.status || "verified", reason: docMap[d]?.reason }))}
              onVerify={verifyDoc}
              onCorrection={correctDoc}
            />
            <Sec>Photos</Sec>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
              {AMB_PHOTOS.map((p, k) => <div key={p} onClick={() => setPhoto(k)} style={{ border: "1px solid var(--border)", borderRadius: 10, height: 74, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, color: "#94A3B8", fontSize: 11, cursor: "pointer" }}><Icon name="Image" size={20} />{p}</div>)}
            </div>
